...
def extract_format_real_data(learning_data: pd.DataFrame, path_length: int, path_bank_size: int, n_stocks: int,
                             ...
                             time_add_type: str = "realistic", preserve_order: bool = True):
    """
    ...
    :preserve_order:            whether to preserve the datapoints in the order of time
    ...
    if preserve_order:
        res[:, :, 1:] = strided_paths[np.sort(indexes)]
    else:

...
def preprocess_real_data(data_kwargs: dict, real_data_kwargs: dict):
    ...
    :param real_data_kwargs:    Specific arguments for real data training. Requires:
                                    - "data_type":             Name of dataset to use
    ...
    try:
        data_type             = real_data_kwargs.get("data_type")
        ...
        preserve_order        = real_data_kwargs.get("preserve_order")
    except KeyError:
    ...
    for i, pair in enumerate(pairs):
        sep = '\t' if data_type=='forex' else ','
        this_df = pd.read_csv(get_project_root().as_posix() + f"/data/{data_type}_data/{pair}_{frequency}.csv", sep=sep)
    ...
    random_indexes = np.arange(np_data.shape[0]) #np.random.permutation(np.arange(np_data.shape[0]))
    if tt_split is None:
...
class TimeSeriesDataset(Dataset):
    ...
    def preprocess(self):
        ...
        for i in range(self.sequences.shape[0]):
            for j in range(self.sequences.shape[2]):
                self.sequences[i, :, j] = pd.Series(self.sequences[i, :, j]).interpolate().fillna(method='bfill').fillna(method='ffill').values

        # Standardize the end time distribution
        end_times = self.sequences[:, -1, :]
        self.mean_end_time = np.nanmean(end_times, axis=0)
        self.std_end_time = np.nanstd(end_times, axis=0)

    def __len__(self):
        return len(self.sequences)

    def __getitem__(self, idx):
        seq = (self.sequences[idx] - self.mean_end_time.reshape((1, -1)))/(self.std_end_time.reshape((1, -1))+1e-7)
        seq /= (seq[0:1, :]+1e-7)
        return torch.tensor(np.concatenate([np.arange(0, self.T, 1).reshape((-1, 1)), seq], axis=-1), dtype=torch.float32),    

def convert_to_sequences(df, N, L, interval=300):
    # Ensure the dataframe is sorted by time
    df = df.sort_values(by='time_int')
    # Extract the relevant columns
    data = df[['value1', 'value2']].values
    time_int = df['time_int'].values
    # Initialize the list to hold the sequences
    sequences = []
    # Generate the sequences
    for start_idx in range(len(data) - L + 1):
        end_idx = start_idx + L
        if end_idx > len(data):
            break
        # Check if the sequence is sampled every 'interval' seconds
        if np.all(np.diff(time_int[start_idx:end_idx]) == interval):
    ...
    if len(sequences) < N:
        raise ValueError(f"Cannot generate {N} sequences of length {L} from the given dataframe. Only {len(sequences)} sequences found.")
    return sequences
    
def process_abilene(path_to_dataset, device, node1, node2, batch_size, num_paths, len_path, train_prop):
    df = pd.read_csv(path_to_dataset)

    def convert_time_to_int(df):
        df['time'] = pd.to_datetime(df['time'], format='%Y%m%d-%H%M')
        df['time'] = df['time'] - df['time'].min()
        df['time_int'] = df['time'].dt.total_seconds() #// 300 # sample_rate = 5 minutes
        return df

    df = convert_time_to_int(df)
    df_filtered = df[((df['src'] == node1) & (df['dest'] == node2)) | ((df['src'] == node2) & (df['dest'] == node1))]
    df_src_to_dest = df_filtered[(df_filtered['src'] == node1) & (df_filtered['dest'] == node2)]
    df_dest_to_src = df_filtered[(df_filtered['src'] == node2) & (df_filtered['dest'] == node1)]
    df_src_to_dest = df_src_to_dest.rename(columns={'Value': 'value1'})
    df_dest_to_src = df_dest_to_src.rename(columns={'Value': 'value2'})
    df_merged = pd.merge(df_src_to_dest[['time', 'time_int', 'value1']], 
                         df_dest_to_src[['time', 'time_int', 'value2']], 
                         on=['time',  'time_int', ], 
                         how='outer')
    df_merged['time_int'] = df_merged['time_int'].astype(int)
    dataset = TimeSeriesDataset(convert_to_sequences(df_merged, num_paths, len_path))
    train_size = int(train_prop * len(dataset))
    test_size = len(dataset) - train_size
    train_dataset, test_dataset = random_split(dataset, [train_size, test_size])
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
    return torch.arange(len_path, device=device), 2, train_dataloader, test_dataloader
